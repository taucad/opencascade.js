...
Reproducibility for an OCCT WASM build means: given the same source revision and the same toolchain, two builds produce byte-identical artifacts. The fork emits three artifacts that, together, let you assert this in CI without trusting any single party:
