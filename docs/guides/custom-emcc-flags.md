...
These are the configurations defined in [`build-configs/configurations.json`](../../build-configs/configurations.json). Apply with `--config <name>`. All four ship with native WASM exceptions, `EVAL_CTORS=2`, and Closure on by default; they differ on opt level, threading, and wasm-opt budget.

| Config                       | Compile | wasm-opt | SIMD | Threading        | Defines                                 | Expected Size |
|------------------------------|---------|----------|------|------------------|-----------------------------------------|---------------|
| `single-threaded` (default)  | `-O3`   | `-O4`    | yes  | single-threaded  | `OCCT_NO_DUMP`, `-UOCC_CONVERT_SIGNALS` | ~21.5 MB      |
| `single-threaded-smallest`   | `-Os`   | `-O3`    | yes  | single-threaded  | `OCCT_NO_DUMP`, `-UOCC_CONVERT_SIGNALS` | ~16.1 MB      |
| `multi-threaded`             | `-O3`   | `-O4`    | yes  | multi-threaded   | `OCCT_NO_DUMP`, `-UOCC_CONVERT_SIGNALS` | ~22 MB        |
| `debug`                      | `-O0`   | `-O0`    | no   | single-threaded  | —                                       | ~35+ MB       |
...
To reduce size, create a custom YAML config with only the symbols your application uses. See `docs/reference/yaml-schema.md` for the full schema and `docs/guides/trim-symbols.md` for the workflow.
