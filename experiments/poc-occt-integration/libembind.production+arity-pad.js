  ...
  $getSignature: (args, keys) => {
    ...
    keys.some(key => {
      ...
      var isKeyMatched = key.every((field, i) => {
        // ── OCJS bounded C1 extension (Gate-1 hunk 3): std::optional<T> wildcard ──
        // EmValOptionalType has `optional: true` and no `registeredClass`; the
        // dispatcher cannot match T against an optional<T>-typed slot. Treat
        // the slot as wildcard; `register_optional<T>::toWireType` either
        // converts the value or accepts undefined as nullopt downstream.
  ...
  $ensureOverloadTable: (proto, methodName, humanName) => {
    if (undefined === proto[methodName].overloadTable) {
      ...
      proto[methodName] = function(...args) {
        // ── OCJS bounded C1 extension (Gate-1 hunk 1): trailing-arg arity padding ──
        // When no registered overload matches call arity exactly, find the
        // smallest registered arity strictly greater than the call arity
        // and pad missing trailing positions with `undefined`. Downstream
        // `register_optional<T>::toWireType(undefined)` → `std::nullopt`,
        // which the binding lambda's `.value_or()` unwraps to the C++
        // default. Non-optional positions throw at toWireType time with a
        // normal type-conversion error.
  ...
  $replacePublicSymbol: (name, value, numArguments, signatureArray, rawSignature) => {
    ...
    if (undefined !== Module[name].overloadTable && undefined !== numArguments) {
      if (undefined !== Module[name].overloadTable[numArguments] && undefined !== Module[name].overloadTable[numArguments].signatures && signatureString) {
        ...
        Module[name].overloadTable[numArguments] = value;
        Module[name].overloadTable[numArguments].signature = signatureString;
        Module[name].overloadTable[numArguments].signatureArray = signatureArray;
      }
    } else {
      Module[name] = value;
      Module[name].argCount = numArguments;
      Module[name].signature = signatureString;
      Module[name].signatureArray = signatureArray;
    }
  },
    ...
    whenDependentTypesAreResolved(
      ...
      (base) => {
        ...
        var constructor = createNamedFunction(name, function(...args) {
          ...
          // ── OCJS bounded C1 extension (Gate-1 hunk 2): trailing-arg arity padding ──
          // Mirrors $ensureOverloadTable: when the call arity has no matching
          // ctor body, find the smallest registered arity strictly greater
          // than the call arity and pad with `undefined`. Required because
          // ctor dispatch lives outside $ensureOverloadTable.
    ...
    whenDependentTypesAreResolved([], [rawClassType], (classType) => {
      ...
      whenDependentTypesAreResolved([], rawArgTypes, (argTypes) => {
        ...
        if (undefined === proto[methodName].overloadTable) {
          ...
          memberFunction.signatureArray = signatureArray;
    ...
    whenDependentTypesAreResolved([], [rawClassType], (classType) => {
      ...
      whenDependentTypesAreResolved([], rawArgTypes, (argTypes) => {
        ...
        if (undefined === proto[methodName].overloadTable) {
          ...
          func.signatureArray = signatureArray;
