...
var LibraryEmbind = {
  ...
  $getSignature: (args, keys) => {
    ...
    keys.some(key => {
      ...
      var isKeyMatched = key.every((field, i) => {
        // ── OCJS bounded C1 extension: std::optional<T> position handling ──
        // When a signature position holds a `register_optional<T>` type,
        // EmValOptionalType is the registration: it has `optional: true`
        // and NO `registeredClass`. Two consequences for C1 dispatch:
        //   1. The existing `args[i] instanceof registeredClass.constructor`
        //      check crashes (`.registeredClass` is undefined).
        //   2. Even when not crashing, the dispatcher cannot match `number`
        //      (or any T) against the optional<T> signature position.
        // We treat optional positions as wildcard-match: dispatch proceeds
        // to the chosen overload, and `register_optional<T>::toWireType`
        // either converts the value to `optional<T>(v)`, accepts `undefined`
        // as `nullopt`, or throws a normal type-conversion error. This
        // preserves the contract that std::optional<T> means "T or absent"
        // without leaking inner-T type info into the dispatcher.
        ...
        return (
          ...
          || (
            ...
            && fieldType !== undefined
            && fieldType.registeredClass !== undefined
            && args[i] instanceof fieldType.registeredClass.constructor
          )
          || typeof args[i] === field
        );
      });
  ...
  $ensureOverloadTable: (proto, methodName, humanName) => {
    if (undefined === proto[methodName].overloadTable) {
      ...
      proto[methodName] = function(...args) {
        // ── OCJS bounded C1 extension: trailing-arg arity padding ─────────
        // When no registered overload matches the call arity exactly, find
        // the smallest registered arity strictly greater than the call
        // arity, and pad the missing trailing positions with `undefined`.
        // Downstream `register_optional<T>::toWireType(undefined)` produces
        // `std::nullopt`, which the binding lambda unwraps via `value_or()`
        // to apply the C++ default. Non-optional positions throw at
        // toWireType time with a normal type-conversion error message.
        //
        // This restores parity with the production bindgen arity fan-out
        // for Option C-style `std::optional<T>` lambdas, allowing one
        // registration per logical overload (vs N today).
        if (!proto[methodName].overloadTable.hasOwnProperty(args.length)) {
          var _arityPadTarget = -1;
          for (var _k in proto[methodName].overloadTable) {
            var _ka = +_k;
            if (_ka > args.length && (_arityPadTarget < 0 || _ka < _arityPadTarget)) {
              _arityPadTarget = _ka;
            }
          }
          if (_arityPadTarget >= 0) {
            while (args.length < _arityPadTarget) args.push(undefined);
          }
        }
        // ──────────────────────────────────────────────────────────────────
        // TODO This check can be removed in -O3 level "unsafe" optimizations.
  ...
  $replacePublicSymbol__deps: ['$throwInternalError', '$throwBindingError'],
  ...
  $replacePublicSymbol: (name, value, numArguments, signatureArray, rawSignature) => {
    ...
    if (undefined !== Module[name].overloadTable && undefined !== numArguments) {
      if (undefined !== Module[name].overloadTable[numArguments] && undefined !== Module[name].overloadTable[numArguments].signatures && signatureString) {
        ...
        if (undefined !== Module[name].overloadTable[numArguments].signatures[signatureString]) {
    ...
    whenDependentTypesAreResolved(
      ...
      (base) => {
        ...
        var constructor = createNamedFunction(name, function(...args) {
          ...
          // ── OCJS bounded C1 extension: trailing-arg arity padding ───────
          // Same rationale as the method dispatcher above (see comment at
          // `$ensureOverloadTable`). When no registered ctor arity matches
          // the call arity exactly, pad to the smallest registered arity
          // strictly greater than the call arity. The padded positions are
          // `undefined`, which `register_optional<T>::toWireType` converts
          // to `std::nullopt`; the ctor lambda's `value_or()` calls then
          // apply C++ defaults. Non-optional positions throw at
          // toWireType time, preserving the existing error semantics for
          // truly malformed calls.
          if (undefined === registeredClass.constructor_body[args.length]) {
            var _arityPadTarget = -1;
            for (var _k in registeredClass.constructor_body) {
              var _ka = +_k;
              if (_ka > args.length && (_arityPadTarget < 0 || _ka < _arityPadTarget)) {
                _arityPadTarget = _ka;
              }
