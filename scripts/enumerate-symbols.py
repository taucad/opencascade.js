...
def enumerate_symbols(occt_root: Path, filter_path: Path):
    ...
    return classes, enums, typedefs, skipped_classes


# ── YAML generation ──────────────────────────────────────────────────

def generate_yaml(classes, enums, typedefs) -> str:
...
def main():
    ...
    classes, enums, typedefs, skipped = enumerate_symbols(occt_root, args.filters)
    ...
    print(f"{'═' * 56}")
    print(f"  Note: Handle_ types are no longer generated.")
    print(f"  Smart pointer handling is via smart_ptr_trait<opencascade::handle<T>>.")
    ...
    content = generate_yaml(classes, enums, typedefs)
