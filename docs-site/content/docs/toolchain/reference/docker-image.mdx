...
| `:branch-<slug>` / `:branch-<slug>-<sha>`                        | Branch tip, single-threaded, `linux/amd64+arm64`, 7-day GHCR retention                |
| `:multi-threaded-branch-<slug>` / `…-<sha>`                      | Branch tip, multi-threaded, `linux/amd64+arm64`, 7-day retention                      |
| `:bindgen-base-branch-<slug>` / `…-<sha>`                        | Branch tip, bindgen-base, `linux/amd64+arm64`, 7-day retention                        |
...
| --------------------------- | ----------------------------- | ----------------------------------------------------------------------------------- |
| Branch, master, and release | `linux/amd64` + `linux/arm64` | `ubuntu-latest` (amd64) + `ubuntu-24.04-arm` (arm64), GitHub Actions native runners |

Every push ships full manifest lists so Apple Silicon and ARM Linux hosts pull
the native architecture transparently. CI links and smokes each image natively,
and requires every native stage to pass before promotion. The amd64 outputs are
the canonical npm package inputs; host-specific compiler output is not compared
byte-for-byte across architectures.
...
The [Dockerfile](https://github.com/taucad/opencascade.js/blob/master/Dockerfile)
