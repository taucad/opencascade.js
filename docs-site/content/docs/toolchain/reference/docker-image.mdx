...
| `:{{version}}-single-threaded`<br/>`:{{version}}-multi-threaded` | Pinned release (for example `:{{version}}-single-threaded`); manifest list of `linux/amd64+arm64` |
| `:{{version}}-bindgen-base`                                      | Pinned release, bindgen-base                                                         |
| `:{{version}}-<stage>` when `{{version}}` is a canary           | Immutable manually dispatched canary (e.g. `:{{version}}-canary.<sha8>-single-threaded`), retained for seven days |
...
this project. Use a version-pinned tag (for example `:{{version}}-single-threaded`) or the
