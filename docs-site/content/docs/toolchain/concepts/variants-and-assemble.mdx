...
It owns the plumbing every consumer would otherwise reimplement:
...
a worker entry and every variant's binary comes with it.

No configuration is needed either way. The glue resolves its `.wasm` sibling relative to its own
