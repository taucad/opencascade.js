...
import { createInstance as createBest } from 'my-occt-package/init';
import { createInstance as createMulti } from 'my-occt-package/multi/init';

const oc = await createBest();                 // best variant this host supports
const mt = await createMulti();                // fixed by the import path
const capped = await createMulti({ threadCount: 4 });
```
...
Importing `multi/init` on a host without cross-origin isolation throws with the
...
```typescript notypecheck
const oc = await createMulti({ threadCount: 8 });
```
