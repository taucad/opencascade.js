...
A reproducible libcascade build means: same inputs, same wasm bytes, every time.
Three controls close the loop.

The repository also runs `.github/workflows/reproducibility.yml` weekly and
on demand. It performs two isolated Linux/amd64 cold builds in parallel,
runs runtime smoke for each, and compares exact artifact ledgers. Stable npm
publication invokes that same exact-commit gate; manually dispatched canaries
and beta builds keep the normal single candidate path and rely on scheduled
cold coverage.
