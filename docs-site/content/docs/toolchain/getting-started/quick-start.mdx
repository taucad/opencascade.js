...
<Callout type="info">
  `@libcascade/toolchain` ships lockstep-versioned with `libcascade` — the
  toolchain that built the binary you depend on is the one whose types describe
  it.
</Callout>
...
`createInstance` owns the plumbing a consumer would otherwise hand-copy: variant
