...
`tests/disposable-containers.test-d.ts`; the generated declarations are the
canonical truth.
...
The v2 name `result` is gone in V3.
...
in the libcascade source repository — when in doubt, the type definitions are the source of
truth.
