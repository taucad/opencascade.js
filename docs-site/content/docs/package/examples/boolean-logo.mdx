---
title: Boolean logo
description: Build a ring glyph with boolean cuts and rotations against a sphere.
---
...
2. `makeCut` constructs a transform (translate + scale), wraps it in a
   ...
   `BRepAlgoAPI_Cut`.
3. Four sequential cuts produce one half of the ring glyph.
4. `BRepAlgoAPI_Fuse` welds the half to a 180°-rotated copy of itself.
5. A final 30° rotation tilts the ring.
