...
thread-pool sizing. Omit `variant` and it selects the most capable variant this
host supports, which is `multi` wherever the capability probe passes. Pass
`threadCount` to cap OCCT's default pool.
...
For most consumers the single-threaded variant wins on simplicity. Force it
with `createInstance({ variant: 'single' })`, or globally before any import:
