...
description: Opt into pthread-enabled OCCT for parallel meshing and boolean workloads via libcascade/multi.
---
...
| --- | --- | --- |
| `libcascade` (default) | `opencascade_full.wasm` | Embeddable widgets, one-op-per-click UX, no COOP/COEP |
| `libcascade/multi` | `opencascade_full_multi.wasm` | Batch mesh/boolean, STEP→glTF pipelines, COOP/COEP-isolated surfaces |

Load the threaded variant the same way as the default — import `libcascade/multi` and resolve wasm through `libcascade/multi/wasm`. See [Bundler & locateFile](./bundler-locatefile#multi-threaded-variant) for Vite, Next.js, and Node recipes.
...
resizable-buffer fast path on the supported browsers.
...
`libcascade/multi` only when you have measured the bottleneck.
