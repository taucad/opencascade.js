...
- **mimalloc** is wired into the WASM allocator so per-thread allocation
  bypasses the global malloc contention point. libcascade supplies OCCT's
  optional `mallinfo` compatibility shim, so custom builds retain strict
  undefined-symbol checking with either mimalloc or Emscripten's allocators.
...
`-sGROWABLE_ARRAYBUFFERS=0` (the default) because runtime support is uneven:
