...
The package exposes the binary through the `ocjs/wasm`
...
Add `ocjs` to `optimizeDeps.exclude` in `vite.config.ts` so
Vite skips its dep-optimizer for the binary module:

```typescript title="vite.config.ts"
import { defineConfig } from 'vite';
export default defineConfig({
  optimizeDeps: { exclude: ['ocjs'] },
});
```
...
const src = fileURLToPath(import.meta.resolve('ocjs/wasm'));
...
Mark `ocjs` as a server external package if you only call it
client-side:

```typescript title="next.config.ts"
import type { NextConfig } from 'next';
const config: NextConfig = {
  serverExternalPackages: ['ocjs'],
};
...
const WASM_DIR = dirname(
  fileURLToPath(import.meta.resolve('ocjs/wasm')),
);
...
Mark `ocjs` as an external or exclude it from aggressive
...
The pthread-enabled build ships under `ocjs/multi` with wasm at `ocjs/multi/wasm`. The `init` contract is identical — only the import path and wasm target change.
...
const src = fileURLToPath(import.meta.resolve('ocjs/multi/wasm'));
await mkdir('public', { recursive: true });
...
const WASM_DIR = dirname(
  fileURLToPath(import.meta.resolve('ocjs/multi/wasm')),
);
