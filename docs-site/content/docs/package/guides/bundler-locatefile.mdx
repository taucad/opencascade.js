...
`createInstance` from `libcascade/single/init` or `libcascade/multi/init` only
when a bundler or deployment copies a fixed variant's binary to another URL.

The package exposes the binaries through the `libcascade/single/wasm` and
`libcascade/multi/wasm` subpath exports (`libcascade/wasm` remains the
single-threaded alias). Every relocation recipe on this page resolves the WASM
through its matching variant specifier; resolving it any other way (deep `node_modules` paths, direct
...
const src = fileURLToPath(import.meta.resolve('libcascade/single/wasm'));
...
const WASM_DIR = dirname(
  fileURLToPath(import.meta.resolve('libcascade/single/wasm')),
);
...
import { createInstance } from 'libcascade/single/init';
import wasmUrl from 'libcascade/single/wasm';
...
The pthread-enabled build is a variant of the same package: import its fixed
initializer from `libcascade/multi/init`. That entry also owns the worker
