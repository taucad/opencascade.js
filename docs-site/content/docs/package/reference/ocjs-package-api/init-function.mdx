...
`init` is the default export of `libcascade`. It instantiates the
...
The `libcascade/wasm` subpath export resolves to the shipped
`opencascade_full.wasm`. The multi-threaded variant uses the same `init`
signature from `libcascade/multi` with wasm at
`libcascade/multi/wasm`. See [Bundler & locateFile](../../guides/bundler-locatefile)
...
Import `libcascade/multi` with wasm at `libcascade/multi/wasm`. The `init` signature is identical; Emscripten pre-spawns pthread workers during instantiation (`PTHREAD_POOL_SIZE=navigator.hardwareConcurrency` is baked into the binary). No extra JS worker wiring is required beyond `locateFile`.
