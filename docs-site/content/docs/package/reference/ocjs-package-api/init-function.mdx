---
title: init() function
description: Default export of ocjs — signature, options, and singleton pattern.
---

`init` is the default export of `ocjs`. It instantiates the
...
The `ocjs/wasm` subpath export resolves to the shipped
`opencascade_full.wasm`. The multi-threaded variant uses the same `init`
signature from `ocjs/multi` with wasm at
`ocjs/multi/wasm`. See [Bundler & locateFile](../../guides/bundler-locatefile)
...
Import `ocjs/multi` with wasm at `ocjs/multi/wasm`. The `init` signature is identical; Emscripten pre-spawns pthread workers during instantiation (`PTHREAD_POOL_SIZE=navigator.hardwareConcurrency` is baked into the binary). No extra JS worker wiring is required beyond `locateFile`.
