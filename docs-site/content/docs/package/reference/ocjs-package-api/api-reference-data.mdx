...
Every published package includes `ocjs/api-reference.json`. The feed is the
portable build-time source for the bound OCCT hierarchy, declarations,
provenance, and exact input hashes used by this site.

```ts
import reference from 'ocjs/api-reference.json' with { type: 'json' };
...
```bash
npm exec nx -- run ocjs:docs-sync -- --from ./ocjs-3.0.0-canary.5bf5e36c.tgz
```
...
```bash
OCJS_API_REFERENCE_SOURCE=../ocjs-3.0.0-canary.5bf5e36c.tgz \
