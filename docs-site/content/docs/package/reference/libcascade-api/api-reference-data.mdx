...
description: Consume the deterministic libcascade API feed at build time.
---
...
Replace `{{version}}` with the package version in the downloaded filename.

```bash
npm exec nx -- run ocjs:docs-sync -- --from ./libcascade-{{version}}.tgz
```
...
```bash
OCJS_API_REFERENCE_SOURCE=../libcascade-{{version}}.tgz \
