...
| `locateFile`           | `import wasmUrl from 'libcascade/wasm?url'` (Vite) / `new URL(import.meta.resolve('libcascade/wasm'))` (Node) / `postinstall` copy into `public/` (Next public assets) | Lets the Emscripten loader find `opencascade_single.wasm` regardless of bundler or runtime              |
