/**
 * Fork-internal shape guard for the `next-three-glb` v3 starter template.
 *
 ...
 * canonical `ocjs` dep, the `ocjs-<name>` package name,
 * the optional CI-gated lockfile, and the canonical memoized-Promise
 * singleton in `lib/ocjs-init.ts`.
 */
...
const TEMPLATE_NAME = 'next-three-glb';
const EXPECTED_PACKAGE_NAME = 'ocjs-next-three-glb';
const INIT_PATH = path.join(TEMPLATE_ROOT, 'lib', 'ocjs-init.ts');
...
describe(`starter-templates/${TEMPLATE_NAME} shape`, () => {
  ...
  describe('lib/ocjs-init.ts canonical memoized-Promise singleton', () => {
    ...
    it('declares the module-scoped `let cached` slot', () => {
      ...
      expect(
        ...
        `lib/ocjs-init.ts must declare a module-scoped \`let cached\` slot. Got first 300 chars:\n${src.slice(0, 300)}`,
      ).toBe(true);
    });
    ...
    it('gates the init call on `if (cached === null)`', () => {
      ...
      expect(
        ...
        `lib/ocjs-init.ts must gate on \`if (cached === null)\`. Got first 300 chars:\n${src.slice(0, 300)}`,
      ).toBe(true);
    });
    ...
    it('invokes init({...}) exactly once', () => {
      ...
      expect(
        matches.length,
        `lib/ocjs-init.ts must invoke init({...}) exactly once outside comments (got ${matches.length}). Code-only source:\n${codeOnly}`,
      ).toBe(1);
    });
  });
});
