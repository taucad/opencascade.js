...
Vite 6 + TS + three.js 0.180+ + `ocjs@canary` via the `ocjs/multi` subpath. Same box-with-hole → GLB → three.js pipeline as [`vite-three-glb`](../vite-three-glb), but loads the pthread-enabled wasm with COOP/COEP headers and activates OCCT parallel mesh/boolean defaults at init.

## Stack

- Vite 6 (`pnpm dev` → http://localhost:3003)
- `ocjs/multi` with wasm at `ocjs/multi/wasm?url`
- `ocjs@canary`, using the `ocjs/multi` and `ocjs/multi/wasm` subpaths
...
| `src/ocjs-init.ts`     | Memoized `ocjs/multi` init + parallel activation    |
