...
- **ES module distribution** — `"type": "module"`, one runtime export (the default `init` function), type-only named exports, and a single `opencascade_single.{js,wasm,d.ts}` triple replacing v2's facade module. Zero-configuration initialization resolves the adjacent WASM; `libcascade/wasm` is the supported relocation subpath for bundlers — see [§A](BREAKING_CHANGES.md#section-a--module-loading).
