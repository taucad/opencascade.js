...
using NCollection_Sequence_int = NCollection_Sequence<int>;EMSCRIPTEN_BINDINGS(IFSelect_Act) {
  class_<IFSelect_Act, base<IFSelect_Activator>>("IFSelect_Act")
    .smart_ptr<opencascade::handle<IFSelect_Act>>("Handle_IFSelect_Act")
    .constructor(optional_override([](std::string name, std::string help, const IFSelect_ActFunc func) {
      return opencascade::handle<IFSelect_Act>(new IFSelect_Act(name.c_str(), help.c_str(), func));
    }))
    .function("Do", &IFSelect_Act::Do, allow_raw_pointers())
    .function("Help",
      std::function<std::string(IFSelect_Act&, const int)>([](IFSelect_Act& that, const int number) -> std::string {
        auto ret = that.Help(number);
    ...
    .class_function("AddFSet",
      ((void (*)(std::string, std::string, const IFSelect_ActFunc))[](std::string name, std::string help, const IFSelect_ActFunc func) -> void {
        IFSelect_Act::AddFSet(strdup(name.c_str()), strdup(help.c_str()), func);
      }
    ), allow_raw_pointers())
    .class_function("get_type_name",
      ((std::string (*)())[]() -> std::string {
        auto ret = IFSelect_Act::get_type_name();
        return ret == nullptr ? std::string() : std::string(ret);
      }
    ), allow_raw_pointers())
    .class_function("get_type_descriptor", &IFSelect_Act::get_type_descriptor, allow_raw_pointers())
    .function("DynamicType", &IFSelect_Act::DynamicType, allow_raw_pointers())
  ;
}
