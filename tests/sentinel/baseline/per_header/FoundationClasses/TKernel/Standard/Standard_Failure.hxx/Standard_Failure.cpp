...
using math_VectorBase_int = math_VectorBase<int>;EMSCRIPTEN_BINDINGS(Standard_Failure) {
  class_<Standard_Failure>("Standard_Failure")
    ...
    .function("what",
      std::function<std::string(Standard_Failure&)>([](Standard_Failure& that) -> std::string {
        auto ret = that.what();
    ...
    .function("GetMessageString",
      std::function<std::string(Standard_Failure&)>([](Standard_Failure& that) -> std::string {
        auto ret = that.GetMessageString();
        return ret == nullptr ? std::string() : std::string(ret);
      }
    ), allow_raw_pointers())
    .function("ExceptionType",
      std::function<std::string(Standard_Failure&)>([](Standard_Failure& that) -> std::string {
        auto ret = that.ExceptionType();
        return ret == nullptr ? std::string() : std::string(ret);
      }
    ), allow_raw_pointers())
    .function("GetStackString",
      std::function<std::string(Standard_Failure&)>([](Standard_Failure& that) -> std::string {
        auto ret = that.GetStackString();
        return ret == nullptr ? std::string() : std::string(ret);
      }
    ), allow_raw_pointers())
    .class_function("DefaultStackTraceLength", &Standard_Failure::DefaultStackTraceLength, allow_raw_pointers())
    .class_function("SetDefaultStackTraceLength", &Standard_Failure::SetDefaultStackTraceLength, allow_raw_pointers())
  ;
}

namespace emscripten { namespace internal { template<> void raw_destructor<Standard_Failure>(Standard_Failure* ptr) { /* do nothing */ } } }
