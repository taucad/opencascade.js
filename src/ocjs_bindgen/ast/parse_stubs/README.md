...
`ocjs_bindgen.ast.parse.parse_additional_bind_code`. The headers here
...
`BUILTIN_ADDITIONAL_BIND_CODE + consumer additionalBindCode` translation
units.
...
If a future OCJS BUILTIN_ADDITIONAL_BIND_CODE block or replicad-style
consumer `additionalBindCode` references a new Embind registration entry
point, add it to:
